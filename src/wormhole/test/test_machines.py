...
def build_boss():
    ...
    wormhole = Dummy("w", events, None, "got_welcome", "got_code",
                     "got_verified_key", "got_app_versions", "received",
                     "closed")
...
def test_boss_basic():
    ...
    b.have_alleged_key()
    b.happy(b"key")
    b.got_app_versions(b"{}")
    b.got_message("0", b"msg1")
    assert events == [
        ("w.got_verified_key", b"key"),
...
def test_scary_phase():
    ...
    # phase=version triggers these two
    b.happy(b"key")
    b.got_app_versions(b"{}")
    events[:] = []
...
def test_unknown_phase(observe_errors):
    # unknown non-numeric should be ignored, but log.err'ed
    b, events = build_boss()
    b.set_code("1-code")
    assert events == [("c.set_code", "1-code")]
    events[:] = []
